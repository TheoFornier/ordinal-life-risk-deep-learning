...
# Charge un modèle TVAE déjà entraîné pour une classe donnée.
def load_model_for_class(class_value: int) -> TVAESynthesizer:
    ...
    return TVAESynthesizer.load(filepath=str(model_path))
...
def main() -> None:
    ...
    plan_b = get_strategy_b_plan(class_counts)

    print("Plan de génération stratégie B :")
    for cls, n in plan_b.items():
        print(f"Classe {cls} -> {n} lignes à générer")

    synthetic_parts = []

    for class_value, num_rows in plan_b.items():
        if num_rows <= 0:
            continue
        ...
        output_class_path = SYNTHETIC_B_DIR / f"synthetic_class_{class_value}_B.csv"
    ...
    synthetic_all_path = SYNTHETIC_B_DIR / "synthetic_tvae_strategy_b.csv"
    ...
    augmented_path = AUGMENTED_DIR / "train_augmented_tvae_strategy_b.csv"
    df_augmented.to_csv(augmented_path, index=False)

    print(f"\nDonnées synthétiques B sauvegardées : {synthetic_all_path}")
    print(f"Dataset augmenté B sauvegardé : {augmented_path}")
    print(f"Taille finale dataset augmenté B : {df_augmented.shape}")
