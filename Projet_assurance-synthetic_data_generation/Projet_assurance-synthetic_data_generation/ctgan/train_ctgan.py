...
# Supprime la colonne Id avant l'entraînement car elle identifie les lignes mais n'apporte pas de structure utile au générateur.
...
# Construit le metadata SDV automatiquement à partir du DataFrame utilisé pour l'entraînement.
...
# Entraîne un modèle CTGAN sur les données d'une seule classe.
...
# Sauvegarde le modèle entraîné dans outputs/models pour pouvoir le recharger ensuite lors de la génération.
def save_model(synthesizer: CTGANSynthesizer, class_value: int) -> Path:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Entraîne un CTGAN pour une classe donnée.")
    parser.add_argument("--class_value", type=int, required=True, help="Classe Response à entraîner")
    parser.add_argument("--epochs", type=int, default=300, help="Nombre d'epochs pour CTGAN")
    parser.add_argument("--batch_size", type=int, default=500, help="Batch size pour CTGAN")
    ...
    synthesizer = train_ctgan_for_class(
        df_class=df_class,
        epochs=args.epochs,
