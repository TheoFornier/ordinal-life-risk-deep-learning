...
from config import TabMConfig, NUM_CLASSES
from loss import soft_qwk_loss
...
class TabMModel(BaseTabularModel):
    ...
    def _build_model(self, X_train_scaled: np.ndarray) -> None:
        ...
        if cfg.use_embeddings:
            ...
            self._bins = compute_bins(X_t, n_bins=cfg.n_bins)
        ...
        self._init_tabm(num_embeddings)

    def _init_tabm(self, num_embeddings) -> None:
        cfg = self.config
    ...
    def fit(
        ...
        use_soft_qwk = cfg.loss_fn == "soft_qwk"
        mse_fn = nn.MSELoss(reduction="none") if not use_soft_qwk else None

        print(f"Loss: {'soft_qwk (temp={:.2f})'.format(cfg.qwk_temperature) if use_soft_qwk else 'mse'}", flush=True)
        ...
        for epoch in range(1, cfg.epochs + 1):
            ...
            for batch in batch_pbar:
                ...
                if use_soft_qwk:
                    pred_flat_denorm = pred_flat * self._y_std + self._y_mean
                    true_flat_denorm = true_flat * self._y_std + self._y_mean
                    loss = soft_qwk_loss(pred_flat_denorm, true_flat_denorm,
                                         temperature=cfg.qwk_temperature)
                else:
                    loss_per = mse_fn(pred_flat, true_flat)
                    loss = (loss_per * w_batch.repeat_interleave(k)).mean() if w_batch is not None \
                        else loss_per.mean()
                loss.backward()
            ...
            with torch.no_grad():
                for X_batch, y_batch in val_loader:
                    ...
                    if use_soft_qwk:
                        pred_flat_denorm = pred_flat * self._y_std + self._y_mean
                        true_flat_denorm = true_flat * self._y_std + self._y_mean
                        batch_loss = soft_qwk_loss(pred_flat_denorm, true_flat_denorm,
                                                   temperature=cfg.qwk_temperature)
                    else:
                        batch_loss = nn.functional.mse_loss(pred_flat, true_flat)
                    val_loss += batch_loss.item() * len(X_batch)
            ...
            if is_best:
                ...
                if run_dir is not None:
                    self.save(os.path.join(run_dir, "best_model.pt"))
                    save_confusion_matrix_plot(
                        run_dir,
                        val_preds_denorm,
                        y_val,
                        title=f"Validation confusion matrix - epoch {epoch} QWK {val_qwk:.4f}",
                    )
                    logger.info(f"Best checkpoint saved. epoch={epoch} val_qwk={val_qwk:.4f}")
            else:
                patience_counter += 1
        ...
        print(f"TabM | best val_loss={best_val_loss:.4f} | best QWK={best_val_qwk:.4f}", flush=True)
        logger.info(f"Training done. best_val_loss={best_val_loss:.4f} best_val_qwk={best_val_qwk:.4f}")
        return history
    ...
    def load(self, path: str) -> None:
        ...
        self._bins = ckpt.get("bins")
        if "input_dim" in ckpt:
            self.input_dim = ckpt["input_dim"]

        cfg = self.config
        num_embeddings = None
        if cfg.use_embeddings and self._bins is not None:
            num_embeddings = PiecewiseLinearEmbeddings(
                self._bins,
                d_embedding=cfg.d_embedding,
                activation=False,
