...
SYNTHETIC_A_DIR = OUTPUT_BASE_DIR / "synthetic_a"
...
# Stratégie A : on remonte seulement les classes rares jusqu'à 50% de la classe majoritaire.
...
def reset_output_dirs() -> None:
    if SYNTHETIC_A_DIR.exists():
        shutil.rmtree(SYNTHETIC_A_DIR)
    SYNTHETIC_A_DIR.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    plan = get_strategy_a_plan(class_counts, rare_ratio=0.5)

    print("Plan stratégie A :")
    ...
    for class_value, n_rows in plan.items():
        ...
        out_path = SYNTHETIC_A_DIR / f"synthetic_class_{class_value}_A.csv"
    ...
    synthetic_all_path = SYNTHETIC_A_DIR / "synthetic_all_strategy_a.csv"
    synthetic_all.to_csv(synthetic_all_path, index=False)

    augmented_df = pd.concat([real_df, synthetic_all], axis=0, ignore_index=True)
    augmented_path = AUGMENTED_DIR / "train_augmented_strategy_a.csv"
    augmented_df.to_csv(augmented_path, index=False)

    print(f"\nDonnées synthétiques A : {synthetic_all_path}")
    print(f"Dataset augmenté A : {augmented_path}")
